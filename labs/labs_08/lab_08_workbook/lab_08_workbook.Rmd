...
# The column names for deaths and prescriptions are obnoxiously littered with capital letters and unnecessary periods. Also the asterisks to denote unavailable data alongside the NA. The deaths AND prescriptions columns are also read in as characters, not numbers.
...
opioid_scrape_clean <- opioid_scrape %>%
  ...
  filter(state != "*Not included - Did not meet inclusion criteria") %>%
  mutate(opioid_death_rate = na_if(opioid_death_rate,"*")) %>%
  # mutate() to transform, we want to take that prescription rate column and use as.numeric to convert all the values to numbers 
...
opioid_scrape_clean <- opioid_scrape %>%
  ...
  # Use mutate to transform using as.numeric all the values in opioid death to numeric values

...
# Q: How many total COVID cases were there in the U.S. on August 30?
# A: There were 5,956,023 total COVID cases in the U.S. on Aug. 30.
...
# Q: How many total COVID deaths were there in the U.S. on August 30?
# A: On Aug. 30 there were 182,045 COVID deaths in the U.S.
...
# Q: How many cases did the average U.S. county have on August 30?
# A: The average U.S. county had 1,907.15 COVID cases on Aug. 30.
...
# Q: How many **deaths** did the average U.S. county have on August 30?
# A: The average U.S. county had 58.29171 COVID deaths on Aug. 30.
...
# A: Vermont had the fewest total COVID cases on Aug. 30. It had 1608 cases.
```
Let's answer another, similar question. 
...
# A: Alaska had the fewest total COVID deaths on Aug. 30. There were 34 deaths.
...
# Video 1 
# Click play to run the codeblock and the video will display in the markdown document. 
embed_youtube("KD-WgV65XCk", width = 500, height=480, frameborder = 0,
allowfullscreen = FALSE, query = NULL)
...
# Write the answer in a comment
# Q: What was the average number of cases per county in each state? Which state had the lowest average?
# A: Vermont had the lowest average number of cases per county. It had 114.86 cases on average on Aug. 30.
...
# A: New Jersey had the highest avg deaths per county on Aug. 30. It had 758.9 deaths on average per county.
...
# Write the answer in a comment
# Q: Which state has the most counties in our data? How many?
# A: Texas has the most counties in our data. It has 251 counties.
```
...
count_counties_per_state <- covid_county %>%
  group_by(state) %>%
  summarise(total_counties = n()) %>%
...
count_counties_per_state

# Q: which state has the smallest number of counties? 
# A: Delaware is the state with the smallest number of counties in our data, it has three total counties. Smaller still are the number of counties in D.C., which isn't a state, but only has one county.
...
```{r}
# Question: What is the average number of counties a U.S. state has?


# Pull from the dataframe we made previously "count_counties_per_state" and then take an average of the counts to get the average number of counties per state in the U.S.
average_counties_in_US <- count_counties_per_state %>%
  summarise(average_number_of_counties = mean(total_counties))

average_counties_in_US

# Answer: The average U.S. state has 61.24 counties.
